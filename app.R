  ...
  tabsetPanel(
    tabPanel("Description",
             tags$div(
               ...
               tags$p("Lamian is a RNA-seq tool used to determine whether there are significant differences in expression across pseudotime (or cell clusters) under different conditions. For specific details, please refer to the introduction by",
                      tags$a(href = "https://github.com/Winnie09/Lamian", target = "_blank", "Professor Wenpin Hou on GitHub.")),
               
               tags$div(
                 style = "background: #e7f3ff; padding: 15px; border-left: 4px solid #007bff; margin: 15px 0;",
                 tags$p(tags$strong("About This Shiny App"), style = "margin-top: 0;"),
                 tags$p("This Shiny app addresses several issues and includes two main sections:"),
                 tags$ul(
                   tags$li(tags$strong("Data Preprocessing"), ": Start here to prepare and process your data."),
                   tags$li(tags$strong("Data Visualization"), ": Use this after running Lamian to generate and save visualizations.")
                 )
               ),
               
               tags$div(
                 style = "background: #fff3cd; padding: 15px; border-left: 4px solid #ffc107; margin: 15px 0;",
                 tags$p(tags$strong("Workflow"), style = "margin-top: 0; color: #856404;"),
                 tags$ol(
                   tags$li("Begin with ", tags$strong("Data Preprocessing"), " to handle your data."),
                   tags$li("Run the Lamian computation (external to this app)."),
                   tags$li("Use the ", tags$strong("Data Visualization"), " section to generate and save results.")
                 )
               ),
               
               tags$p("If you have any questions or need assistance, feel free to",
                      tags$a(href = "mailto:btye@bu.edu", "email btye@bu.edu."))
             )
    ),
...
server <- function(input, output, session) {
      ...
      fluidRow(
        column(3,
               ...
               radioButtons(
                 ...
                 "Run Automatically:",
                 choices = c("Yes" = TRUE,
                             "No" = FALSE), selected = TRUE, inline = TRUE),
               hr(),
  ...
  output$step1_description <- renderUI({
    tags$div(
      ...
      p("c. Click \"INITIATE Normalization\" or \"INITIATE Normalization with Harmony\" to start normalization and harmonization."),
      tags$hr(),
      h4("3. Next Step"),
  ...
  output$step2_description <- renderUI({
    tags$div(
      ...
      p("Select the appropriate Sample index and Cell Cluster. Sample index is generally the sample number and information used to distinguish different samples; Cell Cluster refers to different cell types in biology. After selection, the UMAP plot in the umap dimension will be automatically updated."),
      tags$hr(),
      h4("2. Subset and Rename"),
  ...
  output$step3_description <- renderUI({
    tags$div(
      ...
      p("Waiting for slingshot analysis. After Slingshot finishes, two plots appear: the top shows cell type distribution, and the bottom shows pseudotime distribution in the chosen dimensionality reduction space."),
      tags$hr(),
      h4("3. Select lineage"),
  ...
  output$step4_description <- renderUI({
    tags$div(
      ...
      p("Set the cells to be removed based on the upper and lower quantiles of the distribution. The middle boxplot will automatically update the red dotted line based on the selected quantile. The table following the box plot shows how many cells will be removed based on the Cell Filter criteria."),
      tags$hr(),
      h4("3. Filtering"),
  ...
  output$step5_description <- renderUI({
    tags$div(
      ...
      p("Enter an absolute output path address, and all generated files will be under this file."),
      tags$hr(),
      ...
      p("b. \"Permutation iterations\" set simulation runs; higher gives more accuracy but takes longer."),
      p("c. \"Task duration\" set the task running time. If the time is too short, the task will be aborted midway."),
      p("d. \"Project name\" is the project name you can use on scc, etc. If you are not run on scc, please check \"Run Automatically\" as \"No\" and change the qsub parameter by yourself."),
      ...
      p("f.\"Run Automatically\" use to select whether you wish to directly run the Lamian qsub task. If your computing resources include 28 cores with 18 GB per core, you may select Yes (for example, on SCC). Otherwise, select No and proceed to modify the parameters in the .qsub file."),
      tags$hr(),
  ...
  observeEvent(input$run_lamian, {
    ...
    tryCatch({
      ...
      step5_result <- step5(
        ...
        path_to_lamian = file.path(path_to_shiny,"Lamian"),
      ...
      if(input$auto_run){
        ...
        showModal(
          modalDialog(
            title = "Lamian Ready to run",
            tagList(
              p("The Lamian analysis is now ready to run."),
              p("Please go to the result folder to custom the qsub parameter:"),
              tags$pre(style = "color: red; user-select: text;", file.path(output_file_path,"lamian.qsub")),
              p("After customing paratemeter, using:"),
              tags$pre(style = "color: red; user-select: text;", paste0("qsub ", file.path(output_file_path,"lamian.qsub"))),
              p("to run the lamian workflow.")
            ),
            footer = modalButton("Close"),
            easyClose = TRUE
          )
